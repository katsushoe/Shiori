...
`shiori-v2.2.0-win-x64-setup.msi`をダウンロードして実行します。
...
最新リリースから`shiori-v2.2.0-win-x64.zip`と隣接するSHA-256ファイルを
...
```powershell
$expected = (Get-Content .\shiori-v2.2.0-win-x64.zip.sha256).Split()[0]
$actual = (Get-FileHash .\shiori-v2.2.0-win-x64.zip -Algorithm SHA256).Hash.ToLowerInvariant()
...
.\scripts\Publish-Windows.ps1 -Version 2.2.0
```
...
診断、クライアント設定生成を実行できます。ワークスペース追加はローカル
